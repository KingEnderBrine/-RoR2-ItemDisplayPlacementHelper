...
namespace ItemDisplayPlacementHelper.Assets.ItemDisplayPlacementHelper
{

    [AddComponentMenu("UI/MultipleOptionsDropdown", 35)]
    [RequireComponent(typeof(RectTransform))]
    public class MultipleOptionsDropdown : Selectable, IPointerClickHandler, IEventSystemHandler, ISubmitHandler, ICancelHandler
    {
        ...
        public class OptionData
        {
            ...
            public OptionData(string text, Sprite image, int flagValue)
            {
                this.text = text;
                this.image = image;
        ...
        private static OptionData s_NoOptionData = new OptionData("None", null, 0);
        private static OptionData s_MixedOptionData = new OptionData("Mixed", null, 0);
        private static OptionData s_AllOptionData = new OptionData("All", null, 0);
        ...
        protected MultipleOptionsDropdown()
        {
        }
        ...
        public void RefreshShownValue()
        {
            ...
            if (m_Value == valueMask)
            {
                optionData = s_AllOptionData;
            }
            else if (options.Count > 0 && m_Value > 0)
            {
                for (var i = 0; i < options.Count; i++)
                {
                    var data = options[i];
                    if ((data.flagValue & m_Value) == data.flagValue)
                    {
                        if (((~data.flagValue) & m_Value) > 0)
                        {
                            optionData = s_MixedOptionData;
                        }
                        else
                        {
                            optionData = data;
            ...
            {
                m_Placeholder.enabled = options.Count == 0 || m_Value == 0;
            }
        }
        ...
        public void Show()
        {
            ...
            var canvasList = base.gameObject.GetComponentsInParent<Canvas>(includeInactive: false);
            if (canvasList.Length == 0)
            {
                return;
            }

            Canvas canvas = canvasList[canvasList.Length - 1];
            for (int i = 0; i < canvasList.Length; i++)
            {
                if (canvasList[i].isRootCanvas)
                {
                    canvas = canvasList[i];
                    break;
                }
            ...
            for (int j = 0; j < options.Count; j++)
            {
                ...
                DropdownItem item = AddItem(data, (value & data.flagValue) > 0, componentInChildren, m_Items);
                if (!(item == null))
                {
                    item.toggle.onValueChanged.AddListener(value =>
                    {
                        OntemToggle(value, data.flagValue);
                    });
                    if (item.toggle.isOn)
                    {
                        item.toggle.Select();
        ...
        private DropdownItem AddItem(OptionData data, bool selected, DropdownItem itemTemplate, List<DropdownItem> items)
        {
            ...
            {
                dropdownItem.toggle.isOn = selected;
            }
        ...
        private void OntemToggle(bool on, int flagValue)
        {
            if (on)
            {
                value |= flagValue;
